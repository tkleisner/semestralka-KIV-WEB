...
class db
{
	...
	* Nacist vsechny zaznamy z tabulky.
	...
	/**
	 * Updatovat polozku v databazi
  * @param $table_name nazev tabulky
  * @param $array poel hodnot
  * @param $where_array pole podminek
	 */
	public function DBUpdate($table_name, $array, $where_array)
	{
	   // MySql
	 	$mysql_pdo_error = false;
     ...
     // slozit si podminku s otaznikama
	 	...
	 	foreach ($array as $column => $value)
	 	{
	 		...
	 		// 1) pripravit dotaz s dotaznikama
	 		$query = "update `$table_name` set $insert_columns $where_pom;";
     //echo $query;
     ...
     $statement->bindValue($bind_param_number, $item["value"]);
  ...
  * @param $where_array pole podminek
  */
	function DBDelete($table_name, $where_array){
    $mysql_pdo_error = false;
